...
namespace ctlComponenteCurp
{
    public partial class ctlCurp: UserControl
    {
        ...
        public ctlCurp()
        {
            ...
            cboEstados.DataSource = estadosAbreviacion;
        }
        ...
        {
            get { return cboEstados.SelectedItem.ToString(); }
            set { cboEstados.SelectedItem = value; }
        }

        private void MostrarCurp()
        {
            lblCurp.Text = curpGenerada;
        }
        ...
        private void GenerarCurp()
        {
            ...
            string primerLetraApellido = PrimerApellido.Substring(0, 1);
            char primerVocal = PrimerApellido.Substring(1)
            ...
            char primerApellidoConsonanteInterna = PrimerApellido.Substring(1)
                .FirstOrDefault(c => "BCDFGHJKLMNPQRSTVWXYZ".Contains(c));

            if (primerApellidoConsonanteInterna == '\0') primerApellidoConsonanteInterna = 'X';

            char segundoApellidoConsonanteInterna = SegundoApellido.Substring(1)
                .FirstOrDefault(c => "BCDFGHJKLMNPQRSTVWXYZ".Contains(c));

            if (segundoApellidoConsonanteInterna == '\0') segundoApellidoConsonanteInterna = 'X';   

            char nombresConsonanteInterna = Nombre.Substring(1)
                .FirstOrDefault(c => "BCDFGHJKLMNPQRSTVWXYZ".Contains(c));

            curpGenerada = $"Tu curp es: {primerLetraApellido}{primerVocal}{primerLetraSegundoApellido}{primerLetraNombre}{fechaNacimiento}{genero}{estado}{primerApellidoConsonanteInterna}{segundoApellidoConsonanteInterna}{nombresConsonanteInterna}00";
        }
        ...
        private void txtPrimerApellido_TextChange(object sender, EventArgs e)
        {
            GenerarCurp();
            MostrarCurp();
        }

        private void txtSegundoApellido_TextChange(object sender, EventArgs e)
        {
            GenerarCurp();
        ...
        private void cboSexo_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            GenerarCurp();
            MostrarCurp();
        }

        private void cboEstados_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            GenerarCurp();
            MostrarCurp();
        }
